...
config.WXLink_LastMessageID_2 = 0
config.WXLink_LastMessageID_3 = 0
...
def returnPowerStatus(device, state):
...
################
# Servo PWM Board Initialization 
...
# we don't use these so move to unused pins
anemometerPin = 7
rainPin = 6
#anemometerPin = 26
#rainPin = 21
...
def completeCommand():
        ...
        f = open("/home/pi/SDL_Pi_ProjectCuracao2/state/WeatherCommand.txt", "w")
        f.write("DONE")
        f.close()
...
def processCommand():
        ...
        f = open("//home/pi/SDL_Pi_ProjectCuracao2/state/WeatherCommand.txt", "r")
        command = f.read()
        f.close()
...
def readWXLink(block1, block2):
		...
		if ((len(block1) > 0) and (len(block2) > 0)):
			...
			if (receivedCRC == calculatedCRC):
				...
				print "S/M %d/%d" % (SensorID, MessageID)
				...
				elements = [block1[29], block1[30], block1[31], block2[0]]
                		...
                		WXLink_batteryVoltage = struct.unpack('f', str(block2[1:5]))[0]
                		WXLink_batteryCurrent = struct.unpack('f', str(block2[5:9]))[0]
                		WXLink_loadCurrent = struct.unpack('f', str(block2[9:13]))[0]
                		WXLink_solarPanelVoltage = struct.unpack('f', str(block2[13:17]))[0]
                		WXLink_solarPanelCurrent = struct.unpack('f', str(block2[17:21]))[0]
					
                		auxA = struct.unpack('f', str(block2[21:25]))[0]
	
	
                		print "WXLink batteryVoltage = %6.2f" % WXLink_batteryVoltage
                		print "WXLink batteryCurrent = %6.2f" % WXLink_batteryCurrent
                		print "WXLink loadCurrent = %6.2f" % WXLink_loadCurrent
                		print "WXLink solarPanelVoltage = %6.2f" % WXLink_solarPanelVoltage
                		print "WXLink solarPanelCurrent = %6.2f" % WXLink_solarPanelCurrent
				...
				if (config.WXLink_LastMessageID != MessageID):
					...
					print "WXLink_Data_Fresh set to True"
                		...
                		if (SensorID == 2):
					if (config.WXLink_LastMessageID_2 != MessageID):
						config.WXLink_Data_Fresh = True
						config.WXLink_Data_Fresh_ID = 2
						config.WXLink_LastMessageID_2 = MessageID
						print "WXLink_Data_Fresh set to True"
					...
					if (config.WXLink_LastMessageID_3 != MessageID):
						config.WXLink_Data_Fresh = True
						config.WXLink_Data_Fresh_ID = 3
						config.WXLink_LastMessageID_3 = MessageID
						print "WXLink_Data_Fresh set to True"
		...
		returnList.append(WXLink_batteryVoltage) 
		returnList.append(WXLink_batteryCurrent) 
		returnList.append(WXLink_loadCurrent) 
		returnList.append(WXLink_solarPanelVoltage) 
		returnList.append(WXLink_solarPanelCurrent) 
...
def writeSunAirPlusStats():
        ...
        f = open("/home/pi/SDL_Pi_ProjectCuracao2/state/SunAirPlusStats.txt", "w")
...
def writeWeatherStats():
        ...
        f = open("/home/pi/SDL_Pi_ProjectCuracao2/state/WeatherStats.txt", "w")
...
def sampleAndDisplay():
	...
	if (config.WXLink_Present == False):	
        ...
        if ((config.OLED_Present) and (config.OLED_Power)):
...
def writeWeatherRecord():
	...
	try:
		...
		query = 'INSERT INTO WeatherData(TimeStamp,Sensor_ID, as3935LightningCount, as3935LastInterrupt, as3935LastDistance, as3935LastStatus, currentWindSpeed, currentWindGust, totalRain,  bmp180Temperature, bmp180Pressure, bmp180Altitude,  bmp180SeaLevel,  outsideTemperature, outsideHumidity, currentWindDirection, currentWindDirectionVoltage, insideTemperature, insideHumidity) VALUES(UTC_TIMESTAMP(), %i, %.3f, %.3f, %.3f, "%s", %.3f, %.3f, %.3f, %i, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f, %.3f)' % (config.WXLink_Data_Fresh_ID,as3935LightningCount, as3935LastInterrupt, as3935LastDistance, as3935LastStatus, currentWindSpeed, currentWindGust, totalRain,  bmp180Temperature, bmp180Pressure, bmp180Altitude,  bmp180SeaLevel,  outsideTemperature, outsideHumidity, currentWindDirection, currentWindDirectionVoltage, HTUtemperature, HTUhumidity)
		print("query=%s" % query)
...
def updateRain():
        global lastRainReading, totalRain, rain60Minutes
...
def turnOffOLED():
	...
	powercontrol.setOLEDDisplayPower(False)
...
def turnOnOLED():
	...
	global display
 	if (config.TCA9545_I2CMux_Present):
         	tca9545.write_control_register(TCA9545_CONFIG_BUS0)
		if (config.OLED_Power == False):
			powercontrol.setOLEDDisplayPower(True)
        		RST =27
        		display = Adafruit_SSD1306.SSD1306_128_64(rst=RST, i2c_address=0x3C)
        		# Initialize library.
        		display.begin()
...
def turnOffLoRa():
 	...
 	if (config.TCA9545_I2CMux_Present):
         	tca9545.write_control_register(TCA9545_CONFIG_BUS0)
	powercontrol.setLoRaPower(False)

def turnOnLoRa():

 	if (config.TCA9545_I2CMux_Present):
         	tca9545.write_control_register(TCA9545_CONFIG_BUS0)
	powercontrol.setLoRaPower(True)
...
def printPowerState():
	...
	print "Power Status"
	print "----------------------"
	print returnPowerStatus("OLED Display", config.OLED_Power )
	print returnPowerStatus("Fan Drive", config.Fan_Drive_Power )
	print returnPowerStatus("LoRa Power", config.LORA_Power )
	print returnPowerStatus("AirQuaility Power", config.AirQuality_Power )
	print returnPowerStatus("WiFi Power ", config.WiFi_Power )
	print "----------------------"
...
powercontrol.initialPowerState()
...
scheduler.add_job(Camera.takeAndSendPicture, 'cron', minute=3, second=10, args=["Hour Pic", pwm, 200, 550 ]) 
...
print "-----------------"
# take picture
# Note Servo is on Base I2C Bus
#camera = picamera.PiCamera()
#Camera.takeSinglePicture(camera, pwm, -60,-20,"Test1.jpg")
#camera.close() 
