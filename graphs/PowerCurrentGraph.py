...
def  PowerCurrentGraph(source,days,delay):
	...
	try:
		...
		query = "SELECT TimeStamp, solarCurrent, batteryCurrent, loadCurrent FROM PowerSystem where  now() - interval %i hour < TimeStamp" % (days*24)
		...
		pylab.figtext(.5, .05, ("ProjectCuracao2 Power Current Last %i Days" % days),fontsize=18,ha='center')
		...
		try:
			...
			pyplot.savefig("/home/pi/SDL_Pi_ProjectCuracao2/static/PowerCurrentGraph.png",facecolor=fig.get_facecolor())	
		...
		del t, s, u, v
		gc.collect()
		print("PowerCurrentGrapGraph finished now")
